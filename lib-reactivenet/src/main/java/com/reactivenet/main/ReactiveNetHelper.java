...
public class ReactiveNetHelper {
    ...
    public static void connectActivity(Context context){
        ...
        merge(ReactiveNetwork.observeNetworkConnectivity(context))
        .subscribe(connectivity -> {
            switch (connectivity.state()) {
                ...
                case CONNECTED:{
                    Toast.makeText(context, "网络已连接 CONNECTED", Toast.LENGTH_SHORT).show();
                    break;
                }
                case SUSPENDED:{
    ...
    public static Observable<Connectivity> merge(Observable observable){
